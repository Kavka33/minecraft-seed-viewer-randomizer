...
export function generateWorld(seed: string, size = 48): WorldPreview {
  ...
  const rare = mulberry32(s ^ 0x51ed2701)() < 0.06;
  ...
  for (let y = 0; y < size; y++) {
    ...
    for (let x = 0; x < size; x++) {
      ...
      counts.set(biome.key, (counts.get(biome.key) ?? 0) + 1);
      row.push({ biome, height: h });
    }
    grid.push(row);
  }

  const mid = Math.floor(size / 2);
  const spawnBiome = grid[mid][mid].biome;
  ...
  const stats = [
    ...
    { label: "Terrain", value: grid[mid][mid].height > 0.6 ? "Hilly" : grid[mid][mid].height < 0.4 ? "Coastal / low" : "Rolling" },
    { label: "Water coverage", value: `${Math.round(((counts.get("ocean") ?? 0) / total) * 100)}%` },
  ...
  return { seedNumber: s, grid, spawnBiome, biomeCounts, structures, stats };
}
